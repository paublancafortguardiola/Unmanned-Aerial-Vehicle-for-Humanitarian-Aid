...
def calculate_minimum_flying_altitude_for_a_window(dem_array, theta_A, phi_A, satellites, d_nni, d_nnj, zone):
    ...
    for instant in range(len(times)):
        ...
        minimum_flying_altitude_calculated[:,:,instant] = minimum_flying_altitude

    np.save(zone + ".npy", minimum_flying_altitude_calculated)

    #minimum_flying_altitude_calculated = np.load(zone + ".npy")
...
# definition of the position of all teh satellites constellation
satellites = satellites_angular_position()

#minimum_flying_altitude = 10000*np.ones(dem_array.shape)
#for i,e in enumerate(satellites):
#    print("position of the satellite. theta = ", np.degrees(e.theta_initial), "phi = ", np.degrees(e.phi_initial))
#    minimum_flying_altitude_Si = main(theta_A,phi_A,e.theta_initial,e.phi_initial,dem_array)
#    minimum_flying_altitude = np.minimum(minimum_flying_altitude_Si,minimum_flying_altitude)

#plot(dem_array, minimum_flying_altitude, d_nni, d_nnj)

calculate_minimum_flying_altitude_for_a_window(dem_array, theta_A, phi_A, satellites, d_nni, d_nnj, zone)